...
import com.d_vide.D_VIDE.app.data.remote.responseDTO.BadgesDTO
...
class GetBadges @Inject constructor(
    val repository: UserRepository
){
    operator fun invoke(): Flow<Resource<BadgesDTO>> = flow {
        try {
            emit(Resource.Loading())
            val r = repository.getBadges()
            when(r.code()) {
